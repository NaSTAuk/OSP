...
class SubmitListCategories extends Component<Props, State> {
	public static getDerivedStateFromProps(nextProps: Props, prevState: State): State {
		if (prevState.init && nextProps.userStation && !nextProps.loading) {
			...
			nextProps.categories.forEach((category) => {
				if (category._id) {
					const entry = nextProps.entries
						? nextProps.entries.find((ent) => {
								return (
									ent.categoryId === category._id &&
									nextProps.userStation &&
									ent.stationId === nextProps.userStation._id
								)
						  })
						: undefined
	...
	public render() {
		...
		return (
			<div className="submit">
				...
				{!this.props.review ? <h1>Categories open for entry</h1> : <h1>Review Your Entries</h1>}
	...
	private renderCategories() {
		if (!this.props.award || !this.props.categories) {
			return (
				<div>
					Unknown award. <Link to="/submit">Back to safety</Link>
				</div>
			)
		}
	...
	private renderCategory(award: Award, category: Category) {
		...
		return (
			<List.Item key={category._id} className="item interactive" onClick={() => this.categoryClicked(award, category)}>
				<div style={{ width: '100%', minWidth: '100%' }}>
					...
					<span style={{ float: 'right' }}>
						...
						{category._id ? (
							this.state.awardsEntered.includes(category._id) ? (
								<Tag color="green">Entered</Tag>
							) : (
								<Tag color="red">Not Entered</Tag>
							)
						) : (
							<Tag>Unknown</Tag>
						)}
	...
	private renderDrawer() {
		...
		if (!category.openForReview) {
			return (
				<React.Fragment>
					<p>This category is still under review, check back soon!</p>
				</React.Fragment>
			)
		}
		...
		return (
			<React.Fragment>
				...
				{this.state.activeEntry.techSpecFailures ? (
					<TechSpecIssuesList failures={this.state.activeEntry.techSpecFailures} />
				) : undefined}
