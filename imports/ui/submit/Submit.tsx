...
class Submit extends Component<Props, State> {
	public static getDerivedStateFromProps(nextProps: Props, prevState: State): State {
		if (prevState.init && nextProps.userStation && !nextProps.loading) {
			...
			if (cat) {
				...
				cat.supportingEvidence
					.filter((ev) => ev.type !== SupportingEvidenceType.CALL)
					.forEach((evidence) => {
	...
	private renderEntryForm(categoryId: string) {
		...
		if (!category) {
			return (
				<div>
					Unknown category. <Link to="/submit">Back to safety</Link>
				</div>
			)
		}
		return (
			<div className="mainContent">
				...
				<Form>
					...
					this.props.award &&
					this.props.award.name === 'NaSTA Awards' &&
					!this.isSuperBlooper() ? (
						<Form.Item>
							<p>
							...
							<Modal
								...
								<p>
									We want to make sure that NaSTA is fair for everyone, as such we require that all content entered be
									published within the last year. There is, however, an allowance for 10% of content to be archive
									material.
								</p>
							</Modal>
							<TextArea
