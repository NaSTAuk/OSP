    ...
    public render () {
        return (
            <div key={ this.props.category._id} className='category'>
                ...
                <p>
                    Sort By&nbsp;
                    <b className='sort-by' onClick={ () => this.sortCategory('name') }>Name</b>&nbsp;
                    <b className='sort-by' onClick={ () => this.sortCategory('result') }>Result</b>
                </p>
    ...
    private renderEntriesInCategory () {
		...
		return (
			<List
				...
				dataSource={ list }
				renderItem={ (entry) => <CategoryEntriesListItem setActiveEntry={(activeEntry) => this.props.setActiveEntry(activeEntry)} key={entry.entry._id} entryWithEvidence={entry} results={this.props.results} /> }
				className='list'
			>
