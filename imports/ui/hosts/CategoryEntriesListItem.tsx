...
export class CategoryEntriesListItem extends Component<Props> {
	public render() {
		return (
			<List.Item
				key={this.props.entryWithEvidence._id}
				className="item"
				onClick={() => this.props.setActiveEntry(this.props.entryWithEvidence)}>
				<div className="content">
					...
					<span className="badges">
						...
						{this.props.entryWithEvidence.evidence.some((ev) => ev.type === SupportingEvidenceType.VIDEO) ? (
							<TechSpecsBadge entry={this.props.entryWithEvidence.entry} />
						) : undefined}
	...
	private renderResultBadge(entry: Entry, result?: Result) {
		...
		return (
			<Tag color={position === 1 ? 'gold' : position === 2 ? 'silver' : position < 5 ? 'green' : 'lime'}>
				{position < 3 ? <Icon type="trophy" /> : this.ordinal_suffix_of(position)}
			</Tag>
		)
	}
